...
Settings contains a disabled Push to database placeholder until database connectivity is available.
Recording screenshots is not required. No network service or upload is involved.
...
- `Reference`: the player's selected starting-deck list, explicitly labelled
  ...
  still needs to select the deck they play; this guarantees fidelity to that list.
...
### Follow-up: preserve the existing deck library across updates

The existing library still lives under the application data root's `cache` folder.
It is not migrated by this change. A future migration should atomically copy the
library, user edits, drafts and related metadata into persistent user app-data,
validate the copy, preserve the original backup and record a migration version.
Subsequent updates must merge shipped public decks without replacing user-owned
records. Identity and new match storage already use that persistent location.
...
an appropriate schema/adapter extension.
...
versus inferred evidence, replacement of hypotheses, missing values, UTC dates,
