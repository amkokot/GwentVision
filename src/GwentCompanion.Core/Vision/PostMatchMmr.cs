...
public sealed record PostMatchMmr(int? RatingAfter, int? Change, bool IsFactionRating, string Label, int? SeasonPeak = null,
    bool Confirmed = true, int ReadCount = 3)
{
    ...
    [JsonIgnore] public string Summary => (Confirmed ? "Observed " : "Unconfirmed best read · ") + $"{(IsFactionRating ? "faction MMR" : "MMR (scope unconfirmed)")}" +
